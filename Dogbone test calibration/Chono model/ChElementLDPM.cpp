...
void ChElementLDPM::SetNodes(std::shared_ptr<ChNodeFEAxyzrot> nodeA,
...
void ChElementLDPM:: ComputeAmatrix( Amatrix& Amat, chrono::ChVector<double>X , chrono::ChVector<double>XI ){
...
void ChElementLDPM::GetLatticeField_dt(unsigned int& ind, unsigned int& jnd, ChVectorDynamic<>& mD) {
    ...
    //
    // displacement dofs:
    mD.segment(0, 3) = (nodes[ind]->Frame().GetPos_dt()).eigen();
    // rotational dofs:   	
    mD.segment(3, 3) = (nodes[ind]->Frame().GetWvel_par()).eigen();
    //
    // Second node
    //
    // displacement dofs:
    mD.segment(6, 3) = (nodes[jnd]->Frame().GetPos_dt()).eigen();
    // rotational dofs:   	
    mD.segment(9, 3) = (nodes[jnd]->Frame().GetWvel_par()).eigen();
...
void ChElementLDPM::ComputeStrain(std::shared_ptr<ChSectionLDPM> section, unsigned int& ind, unsigned int& jnd, ChVectorDynamic<>& mstrain) {
	...
	for (int id=0;id<3;id++) {			
			...
			// NodeA
			//
			ChVector<> nn{nmL(id,0), nmL(id,1), nmL(id,2)};	
			nn=q_delta.Rotate(nn);
			//nn=chrono::ChTransform<>::TransformParentToLocal(nn, ChVector<>(0, 0, 0) ,  q_delta);
			n<< nn[0], nn[1], nn[2];
...
void ChElementLDPM::SetupInitial(ChSystem* system) {
    ...
    for( auto facet:this->GetSection()){ 
	    ...
	    ChMatrix33<> A0;
	    ChVector<> mXele = nodes[jnd]->GetX0().GetPos() - nodes[ind]->GetX0().GetPos();
	    ChVector<> myele =
		(nodes[ind]->GetX0().GetA().Get_A_Yaxis() + nodes[jnd]->GetX0().GetA().Get_A_Yaxis()).GetNormalized();
	    A0.Set_A_Xdir(mXele, myele);
	    facet->Set_ref_rot( A0.Get_A_quaternion() );
	    ///
	    ///
	    ///
...
void ChElementLDPM::UpdateRotation() {
    ...
    for( auto facet:this->GetSection()){
		...
		q_lattice_abs_rot =facet->Get_ref_rot();
	    	...
	    	ChVector<> mXele = nodes[jnd]->Frame().GetPos() - nodes[ind]->Frame().GetPos();
	    	ChVector<> myele =
		(nodes[ind]->Frame().GetA().Get_A_Yaxis() + nodes[jnd]->Frame().GetA().Get_A_Yaxis()).GetNormalized();
	    	Aabs.Set_A_Xdir(mXele, myele);
	    	q_lattice_abs_rot = Aabs.Get_A_quaternion();	    	
...
void ChElementLDPM::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementLDPM::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
    ...
    //
    mD.segment(block_offset + 12, 3) = nodes[2]->GetPos_dt().eigen();
    mD.segment(block_offset + 15, 3) = nodes[2]->GetWvel_loc().eigen();
    //
    mD.segment(block_offset + 18, 3) = nodes[3]->GetPos_dt().eigen();
    mD.segment(block_offset + 21, 3) = nodes[3]->GetWvel_loc().eigen();
}
...
double ChElementLDPM::ComputeTetVol(ChVector<> p1, ChVector<> p2, ChVector<> p3, ChVector<> p4){
    double tetvol=0;
...
ChMatrixNM<double,6,6> ChElementLDPM::ComputeTetMassN(std::shared_ptr<ChSectionLDPM> section, ChVector<> pN, ChVector<> pC, ChVector<> pA, ChVector<> pB){
	...
	ChVector<double> pG=(pN+pC+pA+pB)/4.0;
...
void ChElementLDPM::ComputeMmatrixGlobal(ChMatrixRef M) {
	...
	for(auto verts: vertices){	
		unsigned int ind=facetNodeNums(iface,0);
    		unsigned int jnd=facetNodeNums(iface,1);
    		//
    		ChVector<> pNA=this->nodes[ind]->GetX0().GetPos();
		ChVector<> pNB=this->nodes[jnd]->GetX0().GetPos();
    		//
