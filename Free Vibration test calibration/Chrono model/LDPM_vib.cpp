...
int main(int argc, char** argv) {
    ...
    */
    ...
    for (unsigned int i = 0; i < my_mesh->GetNnodes(); i++) {
        auto node = std::dynamic_pointer_cast<ChNodeFEAxyzrot>(my_mesh->GetNode(i));
        auto p = node->GetPos();
        if (abs(p.z() - 200) < 0.1 && abs(p.x() - 100) < 2 && abs(p.y() - 100) < 2) {
            RP1 = node;
        }
    }
    ...
    for (auto node: bottom_nodes) {
        auto constr_tie=chrono_types::make_shared<ChLinkMateGeneric>(true, true, true, true, true, true);//free
        //auto constr_tie=chrono_types::make_shared<ChLinkMateGeneric>(true, true, true, true, true, true);//fixed
    ...
    for (auto node: top_nodes) {
        auto constr_tie=chrono_types::make_shared<ChLinkMateGeneric>(false, false, false, false, false, false);
    ...
    */
    ...
    //std::cout << "RP1 frame" << constr_tie->GetFrame1Rel() << std::endl;
	...
	*/  
    ...
    //if (mystepper==ChTimestepper::Type::HHT){
	    mystepper->SetAlpha(-0.0); // alpha=-0.2 default value
	...
	*/
    ...
    auto motor1 = chrono_types::make_shared< ChLinkMotorLinearForce>();
    motor1->SetGuideConstraint(false, false, false, false, false);
    ...
    auto my_motion_function1 = chrono_types::make_shared<ChFunction_Ramp>(0, 5000.0);
    auto my_motion_function2 = chrono_types::make_shared<ChFunction_Ramp>(0, -1250000.0);
    ...
    f_sequence1->InsertFunct(my_motion_function1, 0.01, 1, true);
    f_sequence1->InsertFunct(my_motion_function2, 0.00004, 1, true);
    //f_sequence1->InsertFunct(my_motion_function3, 3, 1, true);

    motor1->SetForceFunction(f_sequence1);
    sys.Add(motor1);

    //std::vector<int> node_idx = {33,132,91,120,141,99,231,101,93,125,28};
    std::vector<int> node_idx = { 890,1405,1107,2126,2128,2183,2118,2439,1721,2289,988 };
    std::vector< std::shared_ptr<ChNodeFEAxyzrot> > node_set;
    //std::vector< std::shared_ptr<ChLinkMateGeneric> > const_list;
    //auto constr_bc=chrono_types::make_shared<ChLinkMateGeneric>(true, true, true, true, true, true); 
    unsigned int icons = 0;
    for (int n : node_idx) {
        auto node = std::dynamic_pointer_cast<ChNodeFEAxyzrot>(my_mesh->GetNode(n-1));
        node_set.push_back(node);
        std::cout << "node_set" << node->GetPos() << std::endl;
    }
    ...
    while (sys.GetChTime() <= 0.1) {
        ...
        if (stepnum % 1 == 0) {
            ...
            */
            ...
            ChVector<> RP1_disp = RP1->GetPos() - RP1->GetX0().GetPos();


            GetLog() << " t=\t" << sys.GetChTime() << "\ttop_plate disp_z=\t" << motor1->GetMotorPos()
                //<< "\tforce=\t" << F
                //<< "\tforce=\t" << motor1->Get_react_force().x()
                //<< "\tSupport=\t" << constr_bot->Get_react_force().x()
                //<< "\tinternal work\t" << Wint
                //<< "\texternal_work\t" << Wext
                //<< "\tkinetic energy\t" << Ek
                << "\tRP1\t" << RP1_disp.x() << "\t" << RP1_disp.y() << "\t" << RP1_disp.z() << "\tnode_vibx\t";
            ...
            for (auto node : node_set) {
                ChVector<> node_disp = node->GetPos() - node->GetX0().GetPos();
                //ChVector<> node_force = const_list[i]->Get_react_force();
                //ChVector<> node_moment = const_list[i]->Get_react_torque();

                auto node_q = node->Frame().GetRot();
                ChVector<> delta_rot_dir;
                double delta_rot_angle;
                node_q.Q_to_AngAxis(delta_rot_angle, delta_rot_dir);
                if (delta_rot_angle > CH_C_PI)
                    delta_rot_angle -= CH_C_2PI;  // no 0..360 range, use -180..+180	
                ChVector<> node_rot = delta_rot_angle * delta_rot_dir.eigen();
                //GetLog() << node_disp.x() << "\t" << node_disp.y() << "\t" << node_disp.z() << "\t" << node_rot[0] << "\t" << node_rot[1] << "\t" << node_rot[2] << "\t";
                histfile << node_disp.x() << "\t" << node_disp.y() << "\t" << node_disp.z() << "\t" << node_rot[0] << "\t" << node_rot[1] << "\t" << node_rot[2] << "\t";
                
                histfile.flush();
            }
            GetLog() << "\t\n";
            histfile << "n_iter=\t" << n_iter << "\t\n";
