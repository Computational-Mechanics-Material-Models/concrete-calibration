...
CH_FACTORY_REGISTER(ChLinkPointTrifacexyz2xyzrot)

ChLinkPointTrifacexyz2xyzrot::ChLinkPointTrifacexyz2xyzrot() : react(VNULL), s2(0), s3(0), d(0) {}

ChLinkPointTrifacexyz2xyzrot::ChLinkPointTrifacexyz2xyzrot(const ChLinkPointTrifacexyz2xyzrot& other) : ChLinkBase(other) {
    react = other.react;
    s2 = other.s2;
    s3 = other.s3;
    d = other.d;
...
int ChLinkPointTrifacexyz2xyzrot::Initialize(std::shared_ptr<ChNodeFEAxyzrot> anodeA,
    ...
    this->d = collision::utils::PointTriangleDistance(mnodeA->GetPos(), mtriangle.mnodeB1->pos, mtriangle.mnodeB2->pos,
...
void ChLinkPointTrifacexyz2xyzrot::Update(double mytime, bool update_assets) {
...
void ChLinkPointTrifacexyz2xyzrot::IntStateGatherReactions(const unsigned int off_L, ChVectorDynamic<>& L) {
    L(off_L + 0) = react.x();
    L(off_L + 1) = react.y();
    L(off_L + 2) = react.z();
}

void ChLinkPointTrifacexyz2xyzrot::IntStateScatterReactions(const unsigned int off_L, const ChVectorDynamic<>& L) {
    react.x() = L(off_L + 0);
    react.y() = L(off_L + 1);
    react.z() = L(off_L + 2);
}

void ChLinkPointTrifacexyz2xyzrot::IntLoadResidual_CqL(const unsigned int off_L,    // offset in L multipliers
...
void ChLinkPointTrifacexyz2xyzrot::IntLoadConstraint_C(const unsigned int off_L,  // offset in Qc residual
...
void ChLinkPointTrifacexyz2xyzrot::IntToDescriptor(const unsigned int off_v,
...
void ChLinkPointTrifacexyz2xyzrot::IntFromDescriptor(const unsigned int off_v,
                                           ChStateDelta& v,
                                           const unsigned int off_L,
                                           ChVectorDynamic<>& L) {
    if (!IsActive())
...
void ChLinkPointTrifacexyz2xyzrot::InjectConstraints(ChSystemDescriptor& mdescriptor) {
    if (!IsActive())
        return;
...
void ChLinkPointTrifacexyz2xyzrot::ConstraintsBiReset() {
...
void ChLinkPointTrifacexyz2xyzrot::ConstraintsBiLoad_C(double factor, double recovery_clamp, bool do_clamp) {
    // Compute residual of constraint as distance of two points: one is A,
    // other is on triangle at the s2,s3 area coordinates:
    //  C = A - s1*B1 - s2*B2 - s3*B3
    // If an offset d is desired, along normal N, this becomes:
...
void ChLinkPointTrifacexyz2xyzrot::ConstraintsBiLoad_Ct(double factor) {
    // if (!IsActive())
    //	return;
...
void ChLinkPointTrifacexyz2xyzrot::ConstraintsLoadJacobians() {
    double s1 = 1 - s2 - s3;
...
//***OBSOLETE*** will be removed in favor of Int... functions
void ChLinkPointTrifacexyz2xyzrot::ConstraintsFetch_react(double factor) {
...
void ChLinkPointTrifacexyz2xyzrot::ArchiveOut(ChArchiveOut& marchive) {
    //// TODO
}

void ChLinkPointTrifacexyz2xyzrot::ArchiveIn(ChArchiveIn& marchive) {
    //// TODO
}
...
// Register into the object factory, to enable run-time dynamic creation and persistence
CH_FACTORY_REGISTER(ChLinkPointTrifacexyzrot2xyzrot)

ChLinkPointTrifacexyzrot2xyzrot::ChLinkPointTrifacexyzrot2xyzrot() : react(VNULL), s2(0), s3(0), d(0) {}

ChLinkPointTrifacexyzrot2xyzrot::ChLinkPointTrifacexyzrot2xyzrot(const ChLinkPointTrifacexyzrot2xyzrot& other) : ChLinkBase(other) {
    react = other.react;
    s2 = other.s2;
    s3 = other.s3;
    d = other.d;
...
int ChLinkPointTrifacexyzrot2xyzrot::Initialize(std::shared_ptr<ChNodeFEAxyzrot> anodeA,
...
void ChLinkPointTrifacexyzrot2xyzrot::Update(double mytime, bool update_assets) {
    // Inherit time changes of parent class
    ChPhysicsItem::Update(mytime, update_assets);
...
void ChLinkPointTrifacexyzrot2xyzrot::IntStateGatherReactions(const unsigned int off_L, ChVectorDynamic<>& L) {
    L(off_L + 0) = react.x();
    L(off_L + 1) = react.y();
    L(off_L + 2) = react.z();
}

void ChLinkPointTrifacexyzrot2xyzrot::IntStateScatterReactions(const unsigned int off_L, const ChVectorDynamic<>& L) {
    react.x() = L(off_L + 0);
    react.y() = L(off_L + 1);
    react.z() = L(off_L + 2);
}

void ChLinkPointTrifacexyzrot2xyzrot::IntLoadResidual_CqL(const unsigned int off_L,    // offset in L multipliers
                                                ChVectorDynamic<>& R,        // result: the R residual, R += c*Cq'*L
                                                const ChVectorDynamic<>& L,  // the L vector
                                                const double c               // a scaling factor
) {
...
void ChLinkPointTrifacexyzrot2xyzrot::IntLoadConstraint_C(const unsigned int off_L,  // offset in Qc residual
                                                ChVectorDynamic<>& Qc,     // result: the Qc residual, Qc += c*C
                                                const double c,            // a scaling factor
                                                bool do_clamp,             // apply clamping to c*C?
                                                double recovery_clamp      // value for min/max clamping of c*C
...
void ChLinkPointTrifacexyzrot2xyzrot::IntToDescriptor(const unsigned int off_v,
                                            const ChStateDelta& v,
                                            const ChVectorDynamic<>& R,
                                            const unsigned int off_L,
                                            const ChVectorDynamic<>& L,
...
void ChLinkPointTrifacexyzrot2xyzrot::IntFromDescriptor(const unsigned int off_v,
                                              ChStateDelta& v,
                                              const unsigned int off_L,
                                              ChVectorDynamic<>& L) {
    if (!IsActive())
...
void ChLinkPointTrifacexyzrot2xyzrot::InjectConstraints(ChSystemDescriptor& mdescriptor) {
    // if (!IsActive())
    //	return;
...
void ChLinkPointTrifacexyzrot2xyzrot::ConstraintsBiReset() {
    constraint1.Set_b_i(0.);
    constraint2.Set_b_i(0.);
    constraint3.Set_b_i(0.);
}
...
//***OBSOLETE*** will be removed in favor of IntLoadConstraint_C
void ChLinkPointTrifacexyzrot2xyzrot::ConstraintsBiLoad_C(double factor, double recovery_clamp, bool do_clamp) {
    // Compute residual of constraint as distance of two points: one is A,
    // other is on triangle at the s2,s3 area coordinates:
    //  C = A - s1*B1 - s2*B2 - s3*B3
    // If an offset d is desired, along normal N, this becomes:
...
//***OBSOLETE*** will be removed in favor of Int... functions
void ChLinkPointTrifacexyzrot2xyzrot::ConstraintsBiLoad_Ct(double factor) {
    // if (!IsActive())
    //	return;
...
void ChLinkPointTrifacexyzrot2xyzrot::ConstraintsLoadJacobians() {
    double s1 = 1 - s2 - s3;
...
//***OBSOLETE*** will be removed in favor of Int... functions
void ChLinkPointTrifacexyzrot2xyzrot::ConstraintsFetch_react(double factor) {
    // From constraints to react vector:
    react.x() = constraint1.Get_l_i() * factor;
    react.y() = constraint2.Get_l_i() * factor;
    react.z() = constraint3.Get_l_i() * factor;
...
void ChLinkPointTrifacexyzrot2xyzrot::ArchiveOut(ChArchiveOut& marchive) {
    //// TODO
}

void ChLinkPointTrifacexyzrot2xyzrot::ArchiveIn(ChArchiveIn& marchive) {
    //// TODO
}
*/
